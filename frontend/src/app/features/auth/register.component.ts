  ...
  template: `
    <div class="auth-container">
      <div class="auth-card">
        ...
        <p class="mt-3">Already have an account? <a routerLink="/auth/login">Login</a></p>
      </div>
    </div>
  `,
  styles: [`
  ...
  onSubmit() {
    if (this.registerForm.valid) {
      this.authService.register(this.registerForm.value).subscribe({
        ...
        error: (err: any) => alert('Registration failed: ' + (err.error?.[0]?.description || 'Error occurred'))
      });
    }
  }
}
